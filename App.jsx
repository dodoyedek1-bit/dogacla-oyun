...
const CardDisplay = ({ card, type, mode = 'draw', onAction, onModeratorAction, activeAssets, currentTeamId, lang, isMyTurn = true, isHost = false, isTimerPaused = false }) => {
    ...
    const triggerAction = () => {
        if (!isMyTurn && type !== 'moderator') return; 
    ...
    if (!card) return null;
    const isBonus = type === 'bonus'; const isObstacle = type === 'obstacle'; const isFinal = type === 'final'; const isPlaying = mode === 'play'; const isModerator = type === 'moderator';
                ...
                <div className="absolute inset-0 z-10 flex flex-col justify-start p-0">
                    <div className={`relative w-full h-[50%] shrink-0 z-0 overflow-hidden flex items-center justify-center ${isBonus || isModerator ? '' : 'bg-black'}`}>
                         ...
                         {(isBonus || isModerator) ? (
                             isHiddenFromOpponent && !isModerator ? (
                                 ...
                                 videoToRender ? <AssetDisplay src={videoToRender} className={`relative z-10 w-full h-full object-contain object-center transition-transform duration-700 ${isPlaying ? 'scale-110 drop-shadow-[0_0_30px_rgba(255,200,0,0.8)]' : 'scale-100 drop-shadow-[0_0_15px_rgba(255,255,255,0.2)]'}`} alt="Card Media" /> : null
                             )
                         ) : (
                             (isObstacle && isHiddenFromOpponent) ? (
                                 ...
                                 videoToRender && <AssetDisplay src={videoToRender} className="relative z-10 w-full h-full object-contain object-center transition-transform duration-700" alt="Character" />
                             )
                         )}
                    ...
                    <div className={`relative z-30 flex-1 flex flex-col items-center justify-start text-center px-4 pb-4 -mt-6`}>
                         ...
                         {isModerator && isHost ? (
                             ...
                             (isMyTurn) ? (
                                 <button onClick={triggerAction} className={`stagger-item opacity-0 w-full mt-auto py-3 sm:py-4 rounded-xl font-black text-sm sm:text-base tracking-widest uppercase shadow-[0_5px_20px_rgba(0,0,0,0.5)] transition-all active:scale-95 ${isPlaying || isFinal || isModerator ? 'bg-gradient-to-r from-yellow-400 to-yellow-600 text-black border-2 border-white animate-pulse' : 'bg-white text-black'}`}>
...
export default function DogaclaVisualsFinal() {
  ...
  useEffect(() => {
      ...
      if (savedRoom && !roomId && user) { joinRoom(savedRoom, true); }
  ...
  useEffect(() => {
      ...
      if (revealState.isRolling) {
          ...
          timeout = setTimeout(() => { setRevealState(prev => ({ ...prev, isRolling: false })); playSynthSound('success', soundEnabled); }, 1500); 
      } else {
          timeout = setTimeout(() => {
              if (revealState.currentIndex < revealState.count - 1) {
                  ...
                  if (revealState.mode === 'single') startSinglePlayer(revealState.count, revealState.selectedTeams);
                  else createRoom(revealState.count, revealState.selectedTeams);
              }
          }, 3500); 
      }
      ...
      if (showHintModal) {
          ...
          const timer = setInterval(() => {
              setHintTimerLeft(prev => {
                  if (prev <= 1) { clearInterval(timer); if (isMyTurn) closeHint(); return 0; }
                  return prev - 1;
              });
          }, 1000);
  ...
  useEffect(() => {
      ...
      const connectionTimeout = setTimeout(() => { if (!user) setAuthError("Bağlantı zaman aşımına uğradı. İnternetinizi kontrol edin veya 'Tek Oyunculu' devam edin."); }, 5000);
      const initAuth = async () => {
          try {
              const hasCanvasConfig = typeof __firebase_config !== 'undefined' && __firebase_config;
              if (hasCanvasConfig && typeof __initial_auth_token !== 'undefined' && __initial_auth_token) await signInWithCustomToken(auth, __initial_auth_token);
              else await signInAnonymously(auth);
          } catch (e) { setAuthError(e.message || "Giriş reddedildi."); }
      };
      initAuth();
      const unsub = onAuthStateChanged(auth, (u) => { if (u) clearTimeout(connectionTimeout); setUser(u); });
  ...
  const createRoom = async (count, activeTeams) => {
      ...
      try {
          ...
          console.error(err); setAuthError("Oda kurulamadı: " + err.message); setTeamSelectMode(null); setRevealState(prev => ({...prev, isActive: false})); 
      }
  };
  ...
  const joinRoom = async (code, silent = false) => {
      ...
      try {
          ...
          if (docSnap.exists()) { 
              setIsSinglePlayer(false); setRoomId(code); localStorage.setItem('dogacla_last_room', code); setAuthError(null); 
          } else { 
              if (!silent) setAuthError("HATA: Böyle bir oda kodu bulunamadı!"); localStorage.removeItem('dogacla_last_room');
          }
  ...
  const drawCard = (type) => { 
      ...
      if (type === 'bonus' && cardData.id === 'kubo') {
          const randUrl = activeAssets.kubo_scenarios[Math.floor(Math.random() * activeAssets.kubo_scenarios.length)];
          finalCardData = { ...cardData, activeVideoUrl: randUrl };
      }

      syncGame({ cardType: type, characterMood: newMood, activeCard: finalCardData, gameState: 'CARD' }); 
  };
  ...
  const prepareBonus = (bonusIndex) => {
      ...
      if (bonusToPlay.id === 'kubo') {
          const randUrl = activeAssets.kubo_scenarios[Math.floor(Math.random() * activeAssets.kubo_scenarios.length)];
          syncGame({ playingBonus: { ...bonusToPlay, activeVideoUrl: randUrl } });
      } else {
          syncGame({ playingBonus: bonusToPlay });
      }
  };
          ...
          <div className="flex items-center gap-1 sm:gap-2">
              ...
              {isHost && gameState !== 'LOBBY' && gameState !== 'INTRO' && (
                  <button 
                    ...
                    className="p-1.5 sm:p-2 bg-emerald-600/80 hover:bg-emerald-600 rounded-lg text-white transition shadow-[0_0_10px_rgba(16,185,129,0.3)]"
                    title="Moderatör Düdüğü"
                  >

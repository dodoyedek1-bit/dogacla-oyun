...
const AssetDisplay = ({ src, className, style, alt }) => {
    ...
    // Removed hardcoded object-contain so we can use object-cover from props!
    if (isVideo) {
            ...
            >
                ...
                <div className="absolute inset-0 z-10 flex flex-col justify-start p-0" style={{ transform: `translate3d(${contentX}px, ${contentY}px, 0)` }}>
                    ...
                    <div className={`relative z-30 flex-1 flex flex-col items-center justify-start text-center px-10 pb-10 ${isBonus ? '-mt-2 pt-4' : '-mt-8'}`}>
                         ...
                         <div className="w-full mb-4 bg-black/40 border border-[#D4AF37]/30 p-4 rounded-xl shadow-inner">
                            <p className={`text-xl md:text-2xl font-bold leading-tight ${accentColor}`}>
                                "{missionText}"
                            </p>
                         </div>
                         
                         <p className={`italic mb-6 px-2 leading-relaxed text-lg ${isBonus ? 'text-yellow-500 font-bold uppercase tracking-widest' : 'text-gray-300'}`}>
                            {flavorText}
                         </p>
